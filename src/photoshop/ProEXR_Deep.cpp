...
{	
	...
	bool unMult = (gInOptions.alpha_mode == ALPHA_TRANSPARENCY || gInOptions.unmult);
					
	globals->doc_in = new ProEXRdoc_readPS( *(globals->ps_in), &ps_calls, unMult, false);
...
static void DoWriteContinue (GPtr globals)
{
	...
	// graffiti
	header.insert("writer", StringAttribute( string("ProEXR Deep for Photoshop") ) );
	...
	ProEXRdoc_writePS_Deep output_file(ps_out, header, false, &ps_calls, gStuff->documentInfo);
